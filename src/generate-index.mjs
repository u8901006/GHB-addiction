...
let files = [];
try {
  files = readdirSync(docsDir).filter((f) => f.startsWith("ghb-") && f.endsWith(".html")).sort().reverse();
} catch {}

const links = files
  .slice(0, 30)
  .map((f) => {
    const date = f.replace("ghb-", "").replace(".html", "");
    let display;
    try {
    ...
    return `<li><a href="${f}">📅 ${display}</a></li>`;
  })
  .join("\n");
...
<title>GHB Addiction Research &middot; GHB成癮研究文獻日報</title>
<style>
...
<div class="container">
  <div class="logo">💊</div>
  <h1>GHB Addiction Research</h1>
  <p class="subtitle">GHB成癮研究文獻日報 &middot; 每日自動更新</p>
